...
- Los datos se leen/escriben en SQLite vía [`useDailyLog`](../../src/hooks/useDailyLog.ts) con esa fecha.
...
| Marcado | Igual que Inicio: tap en fila alterna tomado/no tomado (`markTaken` en `dailyLogs` para la fecha seleccionada). |
...
| DL-3 | Cambiar la fecha y marcar filas persiste en SQLite para esa fecha. |
