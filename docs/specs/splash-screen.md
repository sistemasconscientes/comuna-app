...
- Apariencia de la barra de estado en runtime: iconos y texto **claros** (_light content_), coherente con **UI oscura** (`theme.bg`, pestañas, pantallas).
...
6. En la raíz de la app (`App.tsx`), `StatusBar` usa **`style="light"`** (`expo-status-bar`), de modo que el contenido de la status bar sea claro sobre fondos oscuros (equivalente a _light content_ en iOS).
